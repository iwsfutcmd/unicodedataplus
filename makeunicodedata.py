...
def maketables(trace=0):
    ...
    makeunicodeprop(unicode, trace)
    makeunicodeunihan(unicode, trace)
...
def makeunicodedata(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("const _PyUnicodePlus_PropertyValueAlias _PyUnicodePlus_PropertyValueAliases[] = {")
        for prop_shortname, prop_ourname in [
            ...
            for value_shortname, aliases in aliases_dict.items():
                if prop_shortname in ('sc', 'blk'):
                    key = aliases[0]
                    fprint('    {"%s", "%s", "%s"},'
                           % (prop_ourname, key, value_shortname))
                    for alias in aliases[1:]:
                        fprint('    {"%s", "%s", "%s"},'
                               % (prop_ourname, key, alias))
...
def makeunicodetype(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        # Generate code for _PyUnicodePlus_IsLinebreak()
        ...
        fprint(" */")
        fprint('int _PyUnicodePlus_IsLinebreak(const Py_UCS4 ch)')
        fprint('{')
        fprint('    switch (ch) {')
...
def makeunicodeprop(unicode, trace):
    ...
    print(len(table), "unique property sets")
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("/* string literals */")
        fprint("static const char *_PyUnicodePlus_ScriptNames[] = {")
        for name in unicode.scripts:
            fprint("    \"%s\"," % name)
        fprint("    NULL")
        fprint("};")

        fprint("static const char *_PyUnicodePlus_BlockNames[] = {")
        ...
        fprint("/* index tables for the database records */")
        fprint("#define PROP_SHIFT", shift)
        Array("prop_index1", index1).dump(fp, trace)
        Array("prop_index2", index2).dump(fp, trace)
...
def makeunicodeunihan(unicode, trace):
    ...
    print(len(table), "unique Unihan property sets")
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("/* index tables for the database records */")
        fprint("#define UNIHAN_SHIFT", shift)
        Array("unihan_index1", index1).dump(fp, trace)
        Array("unihan_index2", index2).dump(fp, trace)
