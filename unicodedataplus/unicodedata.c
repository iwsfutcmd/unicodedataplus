...
typedef struct {
    ...
    const unsigned char grapheme_cluster_break; /* index into
                                                   _PyUnicodePlus_GraphemeBreakNames */
    const unsigned char incb;           /* index into
                                           _PyUnicodePlus_IndicConjunctBreakNames */
...
{
    return PyBool_FromLong(_PyUnicodePlus_IsXidStart(chr));
}
...
{
    return PyBool_FromLong(_PyUnicodePlus_IsXidContinue(chr));
}
...
_Py_NextGraphemeBreak(_PyGraphemeBreak *iter)
{
    ...
    while (iter->pos < iter->end) {
        ...
        const _PyUnicodePlus_DatabaseRecord *record = _getrecord_ex(chr);
...
/*[clinic end generated code: output=ab92c472815421ac input=12fca184c5604436]*/
{
    return unicodedata_extended_pictographic_impl(self, chr);
}
